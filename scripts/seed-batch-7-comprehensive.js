/**
 * Comprehensive Script to seed Batch-7 with complete data structure
 ...
 * 
 * Usage: node scripts/seed-batch-7-comprehensive.js [--dry-run]
...
if (!MONGODB_URI) {
  ...
  console.log('⚠️  Using default MONGODB_URI from script. Consider setting MONGODB_URI in .env.local')
}

// Ensure URI doesn't have duplicate database names
if (MONGODB_URI.includes('/student_management/student_management')) {
  MONGODB_URI = MONGODB_URI.replace('/student_management/student_management', '/student_management')
}
...
const coachesData = [
  { name: 'Dr. Rajesh Kumar', email: 'rajesh.kumar.b7@school.edu', phone: '+91-9876543201' },
  { name: 'Ms. Priya Sharma', email: 'priya.sharma.b7@school.edu', phone: '+91-9876543202' },
  { name: 'Mr. Amit Singh', email: 'amit.singh.b7@school.edu', phone: '+91-9876543203' },
  { name: 'Dr. Sunita Verma', email: 'sunita.verma.b7@school.edu', phone: '+91-9876543204' },
  { name: 'Mr. Vikram Patel', email: 'vikram.patel.b7@school.edu', phone: '+91-9876543205' },
]
...
function generateStudentData(index) {
  ...
  const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}${index}@student.batch7.edu`
...
/**
 * Main function to seed complete Batch-7 data
 */
async function seedBatch7Comprehensive() {
  let client

  try {
    console.log('🚀 Starting comprehensive Batch-7 seeding...')
    ...
    for (const student of createdStudents) {
      ...
      if (!isDryRun && createdStudents.indexOf(student) % 10 === 0) {
        process.stdout.write(`    Progress: ${createdStudents.indexOf(student) + 1}/${createdStudents.length} students\r`)
      }
    }
    ...
    if (skippedAttendanceRecords > 0) {
      ...
      console.log(`${isDryRun ? '📋' : '✅'} ${isDryRun ? 'Would create' : 'Successfully created'} ${totalAttendanceRecords} attendance records (${attendanceDates.length} days × ${createdStudents.length} students)!\n`)
    }
    ...
    console.log(`Attendance Records: ${totalAttendanceRecords} (30 days per student)`)
    ...
    console.log('\n🎉 Batch-7 seeding completed successfully!')
    console.log('\n📝 Next steps:')
    ...
    console.error('❌ Error seeding Batch-7:', error)
    throw error
  } finally {
    if (client && !isDryRun) {
      await client.close()
