 ...
 * Creates: 4 sections, 5 coaches, 5 subjects per section, 30 students per section,
 * 5 tests per subject with marks, and 30 days of attendance
 * Usage: node scripts/seed-batch-7-complete.js [--dry-run]
 ...
 */
async function seedBatch7Complete() {
  let client
  ...
  try {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      const coach = createdCoaches[i % createdCoaches.length] // Assign coaches in round-robin
    ...
    // Final Summary
    console.log(`\n${isDryRun ? '📋 DRY RUN COMPLETE' : '🎉 SEEDING COMPLETE'}! Here's your Batch-7 summary:`)
    ...
    console.error(`❌ Error ${isDryRun ? 'in dry run' : 'seeding Batch-7 data'}:`, error)
    process.exit(1)
  } finally {
    if (client && !isDryRun) {
      await client.close()
