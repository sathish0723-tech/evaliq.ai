...
export default function CopilotChatPage() {
  ...
  const { conversations, loading: isLoadingChats } = useApp()
  ...
  const cleanStreamingText = stripReasoningTrace(streamingText);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const result = await startStream({
        ...
        batch: batch,
        onToken: (accumulated) => {
          // Text is being streamed — UI updates via streamingText state
          // The streaming message bubble is rendered from streamingText
        },
        onComplete: (fullText, metadata) => {
          const rawAssistantContent = fullText || 'Sorry, I could not generate a response.'

          // Remove step-by-step reasoning traces from the final response
          const stripReasoningTrace = (text) => {
            if (!text || typeof text !== 'string') return text;
            if (text.trim().startsWith('> ')) {
              const parts = text.split('---');
          ...
          const cleanAssistantContent = stripReasoningTrace(rawAssistantContent);
          ...
          const assistantMessage = {
            ...
            timestamp: new Date(),
            data: metadata?.data && Array.isArray(metadata.data) ? metadata.data : null,
          }
          ...
          if ((selectedTool === 'deep-search' || selectedTool === 'report') && rawAssistantContent) {
            const titleMatch = cleanAssistantContent.match(/^#\s+(.+)$/m) ||
              cleanAssistantContent.match(/^(.+?)\n/m) ||
              cleanAssistantContent.match(/title[:\s]+(.+)/i)
            const extractedTitle = titleMatch ? titleMatch[1].trim() :
              (selectedTool === 'deep-search' ? 'Deep Search Results' : 'Research Report')

            const reportDataArray = metadata?.data && Array.isArray(metadata.data) ? metadata.data : null

            setReportMessageIds(prev => new Set([...prev, assistantMessage.id]))

            setReportTitle(extractedTitle)
            setReportContent(cleanAssistantContent)
  ...
  const handleStopStreaming = () => {
    ...
    // Save whatever we have so far as the assistant message
    if (cleanStreamingText) {
      const assistantMessage = {
        id: Date.now() + 1,
        role: 'assistant',
  ...
  const formatMessageContent = (content, timestamp) => {
    ...
    // Remove the step-by-step reasoning traces (e.g., "> 🔍 Analyzing...\n> 🧠 Generating...\n---")
    // from the content so they don't clutter the UI now that we have the single-line progressMessage.
    let displayContent = content;
    if (typeof displayContent === 'string' && displayContent.trim().startsWith('> ')) {
      const parts = displayContent.split('---');
      if (parts.length > 1) {
        // Assume anything before the first '---' is the trace if it contains blockquotes
        displayContent = parts.slice(1).join('---').trim();
      } else {
        // No divider, just remove all leading blockquote lines
        displayContent = displayContent.replace(/^\s*(?:>\s*[^\n]*\n\s*)+/, '').trim();
      }
    }

    // Check if content contains HTML/React markup
    const hasHTMLMarkup = /<[^>]+>/.test(displayContent) ||
      /data-radix-/.test(displayContent) ||
      /className=/.test(displayContent) ||
      /data-slot=/.test(displayContent) ||
      /Primitive\./.test(displayContent)
    ...
    const textContent = hasHTMLMarkup ? extractText(displayContent) : displayContent
    const jsonMatches = findJSONObjects(displayContent)
          ...
          {!showReportView && (
            <div className="flex flex-col h-full relative transition-all duration-300 w-full overflow-hidden overflow-x-hidden">
              ...
              <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 pb-64 space-y-4 w-full max-w-3xl mx-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgb(63 63 70) transparent' }}>
                          ...
                          {isReportMessage ? (
                            <Card
                              ...
                              onClick={() => {
                                ...
                                const cleanContent = stripReasoningTrace(message.content);

                                const titleMatch = cleanContent.match(/^#\s+(.+)$/m) ||
                                  cleanContent.match(/^(.+?)\n/m) ||
                                  cleanContent.match(/title[:\s]+(.+)/i)
                                const extractedTitle = titleMatch ? titleMatch[1].trim() :
                                  (selectedTool === 'deep-search' ? 'Deep Search Results' : 'Research Report')
                              ...
                              <div className="p-4">
                                ...
                                <p className="text-xs text-muted-foreground mt-2 line-clamp-2">
                                  {(() => {
                                    ...
                                    return stripReasoningTrace(message.content).substring(0, 150) + '...';
                                  })()}
                                </p>
                              </div>
                            </Card>
                ...
                {!isLoadingConversation && isStreaming && (
                      ...
                      <div className="max-w-[80%] rounded-lg p-3 text-sm break-words bg-transparent text-foreground overflow-x-hidden">
                        {cleanStreamingText ? (
                          ...
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <span>
                              {progressMessage || (selectedTool === 'deep-search'
                                ? 'Deep searching across data...'
                                : selectedTool === 'report'
                ...
                {/* Non-streaming loading indicator (fallback) */}
                {!isLoadingConversation && isLoading && !isStreaming && (
                  <div className="flex gap-3 justify-start">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="mt-1">
                      <path d="M10 2L12 8L18 10L12 12L10 18L8 12L2 10L8 8L10 2Z" fill="#4285F4" />
                      <path d="M10 6L11 9L14 10L11 11L10 14L9 11L6 10L9 9L10 6Z" fill="#34A853" />
