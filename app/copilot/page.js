...
export default function CopilotPage() {
  ...
  const { user, conversations, loading: isContextLoading, fetchConversations } = useApp()
  ...
  useEffect(() => {
    if (streamingPreviewRef.current && cleanStreamingText) {
    ...
    try {
      ...
      const result = await startStream({
        ...
        onComplete: async (fullText, metadata) => {
          // Stream finished — save conversation and navigate
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12">
          ...
          <div className="flex items-center gap-2 px-4">
            <Select
              ...
              disabled={isContextLoading || conversations.length === 0}
            >
              <SelectTrigger className="w-[200px]">
                <div className="flex items-center gap-2">
                  <History className="h-4 w-4" />
                  <SelectValue placeholder={isContextLoading ? 'Loading...' : conversations.length === 0 ? 'No chat history' : 'Chat History'} />
                </div>
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="flex flex-1 flex-col h-[calc(100vh-4rem)] relative">
          ...
          <div className="flex flex-col items-center justify-center flex-1 px-4 pb-20">
            ...
            {showStreamingPreview && (
              <div className="w-full max-w-3xl mb-6 animate-in fade-in duration-300">
                <div className="flex gap-3 w-full">
                  ...
                  <div
                    ...
                    className="flex-1 max-h-[60vh] overflow-y-auto rounded-lg p-3 text-sm text-foreground"
                  ...
                  >
                    {cleanStreamingText ? (
                      <div className="whitespace-pre-wrap">
                        {cleanStreamingText}
                        {isStreaming && (
                          <span className="inline-block w-2 h-4 ml-0.5 bg-primary align-middle" style={{ animation: 'blink 0.7s step-end infinite' }} />
                        )}
                      </div>
                    ) : (
