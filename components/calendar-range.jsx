  ...
  const renderCalendar = () => {
    ...
    for (let i = prevMonthTotal - prevMonthDays + 1; i <= prevMonthTotal; i++) {
      ...
      days.push(
        <button
          key={`prev-${i}`}
          className={`h-9 w-full flex items-center justify-center text-xs transition-colors ${(isStart && isEnd) ? 'rounded-full' :
    ...
    for (let i = 1; i <= totalDays; i++) {
      ...
      days.push(
        <button
          key={`current-${i}`}
          className={`h-9 w-full flex items-center justify-center text-xs transition-colors font-medium ${(isStart && isEnd) ? 'rounded-full' :
            isStart ? 'rounded-l-full' :
              isEnd ? 'rounded-r-full' :
                inRange ? 'rounded-none' :
                  'rounded-full'
          ...
          <div className="space-y-0.5">
            {presets.map((preset) => (
              <button
                ...
                className={`w-full text-left px-2 py-1.5 text-xs rounded transition-colors ${selectedPreset === preset.key
