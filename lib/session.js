 ...
 * Checks NextAuth session first, then falls back to old session system
  ...
  try {
    ...
    try {
      ...
      if (nextAuthSession?.user) {
        // Convert NextAuth session to old format for compatibility
        return {
          userId: nextAuthSession.user.id,
          email: nextAuthSession.user.email,
          role: nextAuthSession.user.role,
          managementId: nextAuthSession.user.managementId,
        }
      }
    } catch (nextAuthError) {
      // NextAuth not available or error, fall back to old system
      console.debug('NextAuth session not available, using old session system')
    }
